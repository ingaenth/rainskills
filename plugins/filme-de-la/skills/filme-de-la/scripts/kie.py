...
"""Cliente kie.ai para o pipeline do filme de la.
    ...
    kie.py base    <img_url> <ref_url> "<prompt>" [--dur 6] [--out cena.mp4]
...
def playground(modelo, entrada, rotulo):
    ...
    # grava o id ANTES de esperar: se a rede cair, o trabalho pago nao se perde
    with open(os.path.join(os.getcwd(), "ids.log"), "a") as _f:
        _f.write(f"{modelo}\t{t}\t{rotulo}\n")
...
def main():
    ...
    p = sub.add_parser("base"); p.add_argument("img"); p.add_argument("ref")
    p.add_argument("prompt"); p.add_argument("--dur", default="6"); p.add_argument("--out")
    p.add_argument("--ar", default="16:9")
    ...
    if a.cmd == "upload":
        ...
        urls = [a.img] + ([a.ref] if a.ref and a.ref != "-" else [])
